...
export default (pool, auth) => {
  router.post('/start', auth, async (req, res) => {
    try {
      ...
      await pool.query("UPDATE players SET game_data = jsonb_set(COALESCE(game_data, '{}'), '{snakeGame}', $1::jsonb) WHERE wallet = $2", [JSON.stringify(newMg), wallet])
