...
function fixItem(item) {
  ...
  for (const stat of Object.keys(item.stats)) {
    ...
    if (range) {
      ...
      if (item.stats[stat] !== newVal) { item.stats[stat] = newVal; changed = true; }
    } else if (item.stats[stat] > cap) {
      item.stats[stat] = cap; changed = true;
    }
  }
  return changed;
}
...
for (const p of players) {
  ...
  if (gd.equippedArtifacts) {
    for (const slot of Object.keys(gd.equippedArtifacts)) {
      if (fixItem(gd.equippedArtifacts[slot])) { pc = true; totalFixed++; }
    }
  }
  if (gd.items && Array.isArray(gd.items)) {
    for (const item of gd.items) {
      if (item.stats && fixItem(item)) { totalFixed++; pc = true; }
    }
  }
  if (pc) {
